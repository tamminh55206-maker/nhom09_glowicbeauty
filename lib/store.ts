...
const migrateAuthState = (persistedState: unknown): AuthPersistedState => {
  ...
  const currentUser = isStoredAccount(persistedState.currentUser)
    ? toAuthUser(ensureStoredAccountId(persistedState.currentUser))
    : isAuthUser(persistedState.currentUser)
      ? ensureAuthUserId({
          ...persistedState.currentUser,
          maNguoiDung:
            accounts.find(
              (account) =>
                account.soDienThoai === persistedState.currentUser.soDienThoai ||
                account.email.toLowerCase() === persistedState.currentUser.email.toLowerCase(),
            )?.maNguoiDung ?? persistedState.currentUser.maNguoiDung,
        })
      : null;
